...
public class DNALinkedList {
	...
	private boolean matches(Node<Character> startNode, String target) {
		for (int i = 0; i < target.length(); i++) {
			if (startNode == null || target.charAt(i) != startNode.getData()) {
				return false;
			}
	...
	// times in this list, returns the first occurrence. If the target is not in
	// this list, returns null.
	public Node<Character> find(String target) {
		// Hint: call matches(). A lot.
		Node<Character> findnode = this.head; // start at the head

		while (findnode.getNext() != null) {
			if (matches(findnode, target)) {
				return findnode;
			}
	...
	public DNALinkedList extract(Node<Character> firstExtractedNode,
			Node<Character> lastExtractedNode) {
	...
	// Assumes insertionPoint is not the head or tail.
	...
	// into this list immediately before target. Throws IllegalArgumentException
	// if can't find transposon or target.
	public void transpose(String transposon, String target) {
		...
		DNALinkedList transposonList = extract(firstNodeOfTransposon,
				lastNodeOfTransposon);
